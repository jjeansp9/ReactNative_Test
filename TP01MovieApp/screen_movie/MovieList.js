...
export default MovieList = (props)=>{
    ...
    // 이 함수형 컴포넌트가 화면에 보여질 때 자동 실행되는 함수 등록
            ...
            headerLeft: ()=>{
                return (
                    <TouchableOpacity 
                        ...
                        marginLeft:16, alignItems:'center'}} 
                        onPress={()=>{
                            // 디바이스에 저장된 사용자 로그인 이메일
    ...
    return (
        // 리스트해야 할 내용이 많이에 Scroll 가능한 컴포넌트
        <ScrollView style={style.root}>
            {/* 큰 이미지로 높은 선호도를 가진 영화들을 가로 스크롤(페이징)로 보여주기 */}
            {/* 이 문서에서 네트워크 fetch 작업을 하기에는 코드가 너무 복잡하여 별도의 Component로 분리하여 작업 [components_movie 폴더에 제작] */}
            <BigCatalogList
                url={bigUrl}
                onPress={(id)=>{
            ...
            <SubCatalogList title='평점순' url={ratingUrl} onPress={(id)=>props.navigation.navigate('MovieDetail',{id:id})}></SubCatalogList>
